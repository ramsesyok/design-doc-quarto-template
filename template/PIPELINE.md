...
**PDF は Quarto 1.9.20 以上でないと出ない**（安定版なら 1.10 系以降）。それより前の
版は book プロジェクトの typst 出力に対応しておらず、`--to typst` を付けても
`WARN: The typst format is not supported by book projects` と出て何も生成されない。
HTML は古い版でも出る。

| Quarto | 同梱 typst | book → PDF |
|---|---|---|
| 1.4.557 / 1.5.57 / 1.6.42 / 1.7.32 / 1.8.24 / 1.9.10 / 1.9.15 | 〜0.13.0 | ✕（警告だけ出て無生成） |
| 1.9.20 / 1.9.38 | 0.14.2 | ○ |
| 1.10.18 / 1.11.0 | 0.15.1 | ○ |

いずれも実測。対応が入ったのは同梱 typst が 0.14.2 になった版と一致する。
`template/` 側では回避できない（テンプレートを一切使わない素の book プロジェクトでも
同じ警告になることを確認済み）。

**動作確認の範囲**: サンプル `docs/`（mermaid・IPO・横向き・分割表・相互参照を含む
全48ページ）を 1.10.18 で通しビルドし、様式の再現を確認している。
