...
> **このファイルは役割を終えました。** 執筆者に配るのは**利用マニュアル**
> （`manual/` をビルドした PDF / HTML）です。記法は利用マニュアルの 6〜10章に
> 移してあり、以後はそちらだけを更新します。このファイルは参照用に残置しています。
...
表そのものの書き方は2種類あります。**`.tbl` はどちらにも同じように使えます**
（採番・相互参照・列幅・分割・セル結合はすべて共通）。違うのは**セルに何を書けるか**だけです。
...
| 書く手間 | 軽い | 罫線を桁で揃える必要あり（全角混じりは手間） |

> **おすすめ: セルの中身が1行で収まらない表（改行・箇条書き・番号付きリストがある）は、
> グリッド表で書く。** パイプ表は「1セル1行」で収まる表に限ります。
> こうしておくと、あとから箇条書きを足したくなったときに**表ごと書き直さずに済みます**
> （パイプ表のセルにはリストを入れられないため、その時点で作り直しになります）。

- 判断は**セル単位ではなく表単位**です。1つの表の中でパイプ表とグリッド表は混ぜられません。
- 既存の `<br>` 入りパイプ表は**そのまま動きます**。遡って直す必要はありません
  （これは推奨であって `<br>` の禁止ではなく、パイプ表に後から1か所だけ改行が要りになった
  ときの逃げ道としては引き続き有効です）。
- 逆に、**1セル1行で収まる表までグリッド表にする必要はありません**。罫線を揃える手間に
  見合いません。
...
- **改行の入る表は、はじめからグリッド表で書くのがおすすめです**
  （[パイプ表とグリッド表の使い分け](#パイプ表とグリッド表の使い分け)）。パイプ表の `<br>` は
  「1セル1行の表に後から1か所だけ改行が要りになった」ときの逃げ道と考えてください。
...
- **グリッド表でも同じように使えます**（セルに箇条書きを入れた階層表も作れます）。
  ただし**横結合・縦結合を手で書いたグリッド表では効きません**（既存の結合を壊さないため）。
...
- **`merge-cols`（自動のセル結合）はグリッド表でも効きます。** ただし**横結合・縦結合を
  手で書いたグリッド表では効きません**（すでにある結合を壊さないよう、安全側に倒して
  何もしません。警告は出ません）。結合を手で書いていないグリッド表なら、パイプ表と
  まったく同じように `merge-cols="1,2"` などが使えます。
- **注意**: 罫線を**桁で揃える**必要があり、とくに全角文字が混ざると揃えるのが手間です。
  1セル1行で収まる表はパイプ表のままにするのがおすすめです
  （[パイプ表とグリッド表の使い分け](#パイプ表とグリッド表の使い分け)）。
...
| 表の書き方の選択 | 1セル1行ならパイプ表、改行・箇条書きが入るならグリッド表（表単位で決める） |
...
| セル結合 | `::: {.tbl merge-cols="all"}` 〜 `:::`（列指定は `merge-cols="2,3"`。グリッド表でも可） |
