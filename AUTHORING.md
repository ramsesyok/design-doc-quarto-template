...
書けます。用途ごとにクラスを使い分ける必要はありません（従来の `.split-table` /
`.merge-rows` も後方互換で使えますが、新規は `.tbl` を推奨）。
...
- **分割表・セル結合**は、その `:::` に `widths=` を足すだけです
  （`::: {.split-table caption="…" widths="…"}` / `::: {.merge-rows widths="…"}`）。

> **ヒント（通常表と分割表を同じ記法で）**: `.split-table` は**表が1つだけのときは
> 「（1／N）」を付けず**、通常の番号つき表と同じ体裁で出ます。参照 `label=` も付くので、
> 通常表も `::: {.split-table caption="…" label="tbl-x" widths="…"}` で統一して書けます
> （表を増やせばそのまま分割表になります）。
...
#### 結合する列を指定する（`merge-cols`）

既定では**左端の列から順に**階層とみなします。そのため **1列目に連番**があるなど左端を
階層に含めたくないときは、`merge-cols="2,3"` で**結合対象の列（1始まり）**を明示します。

```markdown
::: {.merge-rows merge-cols="2,3"}
| No | 大分類 | 中分類 | 項目         |
|:--:|--------|--------|--------------|
| 1  | 受注   | 登録   | 入力チェック |
| 2  | 受注   | 登録   | 重複チェック |
...
- 分割表と併用（`.split-table .merge-rows merge-cols="2,3"`）するときも同じ属性が効きます。
...
```markdown
::: {.split-table caption="ユーザ属性一覧" label="tbl-user-split"}
| 属性名 | 型     | 必須 | 説明         |
|--------|--------|------|--------------|
| 氏名   | 文字列 | ○   | 利用者の氏名 |
| 年齢   | 数値   |      | 満年齢       |
...
- **セル結合と併用**するときは `.merge-rows` を併記します。分割した各パートで、縦に続く
  同じ値のセルが rowspan 結合されます（上の「セル結合」を分割表に適用する形）:

  ```markdown
  ::: {.split-table .merge-rows caption="機能分類" label="tbl-cls-split"}
  … 空行区切りの複数パイプ表 …
  :::
  ```
...
- **横向きにするときは `.landscape` で入れ子**にします:
  ...
  ```markdown
  ::: {.landscape}
  ::: {.split-table caption="…" label="tbl-id"}
  … …
  :::
  :::
  ```
...
- **相互参照の ID** … `label="tbl-x"` 属性で指定（省略可・`tbl-` 始まり必須）
...
- 入力・出力欄は幅が可変なので、ネストした箇条書きも収まります。**箇条書き（`-`）は
  表示上は記号なし・左詰め**になり、欄を広く使えます（番号付き `1.` は番号が残ります）。
...
| セル内の改行 | `<br>` または `<br/>`（表・グリッド表とも、PDF・HTML 両対応） |
| 列幅の指定 | `::: {widths="10,20,70"}` 〜 `:::`（`.tbl`/分割/結合は `:::` に `widths=` を追記） |
| セル結合 | `::: {.merge-rows}` 〜 `:::`（列指定は `merge-cols="2,3"`） |
| 表のページ分割 | `::: {.split-table caption="…" label="tbl-id"}` ＋ 空行区切りの複数パイプ表 〜 `:::` |
| 分割＋セル結合 | `::: {.split-table .merge-rows caption="…" label="tbl-id"}` 〜 `:::` |
