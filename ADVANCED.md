...
README は「設計書を書く人」向けの最短手順にしてあります。このファイルには、その裏側
（リポジトリの構成・ビルドの仕組み・mermaid を PDF に載せるための準備・環境変数）を
まとめています。**設計書を書くだけなら読む必要はありません。**

- 執筆の記法 … [AUTHORING.md](AUTHORING.md)
...
記録します。どちらも `.gitignore` 済みです。`template/lib.typ` や `design-doc.css` を
改修したら `setup` を再実行してください（何度実行しても同じ結果になります）。
ビルドスクリプトは先頭で `setup` を呼ぶので、スクリプト経由なら再実行は自動です。

## 2. 執筆フォルダ（docs）を改名したいとき
...
- **納品 PDF と配布 HTML（`build-qmd` / `build-html`）** … mermaid-cli でベクター SVG に
